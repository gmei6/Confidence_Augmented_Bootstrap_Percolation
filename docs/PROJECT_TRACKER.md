...
- **Last updated:** 2026-06-03 — Session 9 (survivor-hazard removed → D-007; §10 next-action fear param → window-length; F2 open)
- **File version:** v1.6
...
- **F2 — The $m$/$k$ "fear marks" mechanism is currently UNDERSPECIFIED and conflicts with §3.3.**
  The original summary added: *"if individual fear is activated, $m$ fear marks are added to the bank,
  lasting $k$ iterations."* This implies fear feeds *marks* (decaying over $k$ rounds) that count
  toward the solvency threshold $r$ — unifying the two channels through the mark mechanism — rather
  than failing the bank directly. **The entire mentor analysis ($R_{\text{fear}}\approx\mu$,
  subcritical amplifier, the mean-field check) assumes the simple direct-failure rule of §3.3.**
  Decision needed: (a) drop $m,k$ and keep direct fear failure (cleanest, matches the analysis,
  recommended for MVP), or (b) specify the marks variant fully and treat it as a separate model.
  Until decided, **implement §3.3 as written** and leave $m,k$ out.
...
- **Phase:** End of Week 1, entering Week 2. Wk-1 milestone remains complete (§6 box checked). F1 resolved (D-005, D-006); survivor-hazard removed (D-007, 2026-06-03); F2 ($m,k$ marks) still open.
...
- **This session (S-009) — decisions only (no code committed):** removed survivor-hazard entirely (D-007); reworked the §10 next-action from a `fear_field` switch to a `window_len` (D-006 window length $X$) parameter.
...
- **Forks awaiting decision:** F2 ($m,k$ marks — drop or specify) — see §3.6. *(Resolved: F1 → incremental (D-005, D-006); F3 → Beta (D-002); E1 → standalone executable + file I/O (D-003); F4 → Janson regime (D-004). Survivor-hazard removed entirely (D-007).)*
...
1. **Resolve F2:** decide on the $m,k$ "fear marks" mechanism — (a) drop for the MVP (recommended by §3.6; the D-006 window family already provides a fear-persistence handle without touching the solvency threshold) or (b) specify fully as a separate model variant.
...
3. **Begin Wk-2 (GO/NO-GO):** 1-D $\mu$-sweep at near-critical $r$ — *does $\mu$ have teeth?* — plus bimodality histograms of $|A^*|/n$ to justify $\theta$. D-004 Janson $p_n$ scaling already applied in the test.
4. Confirm `requirements.txt` pins exact versions (§5.4); commit the repo with `.gitignore`, test files, and `pytest.ini`.
5. Email Prof. Dhara re PACE; put Q1–Q3 on the first-meeting agenda.
