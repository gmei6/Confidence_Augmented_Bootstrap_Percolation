...
def test_pair_work_grows_slower_than_quadratically():
    """The point of the port, asserted directly rather than through a stopwatch.

    Wall-clock timing in a test is flaky on a shared machine, so this counts how
    many pairs the sampler actually evaluates — the quantity the algorithm exists
    to reduce. The reference evaluates n(n-1)/2 of them by construction, so a 4x
    increase in n multiplies its work by ~16. The indexed sampler is required to
    stay under 6x over the same step, which is far above what it does in practice
    and far below quadratic; the gap is deliberate, so this test reports a real
    regression rather than ordinary variation between machines.
    """
    import twocascade.girg as girg_mod

    evaluated = {}
    original = girg_mod._torus_cell_gap
    for n in (500, 2000):
        points, weights = _instance(n, 55)
        calls = {"k": 0}

        def counting_gap(a, b, m, _o=original, _c=calls):
            _c["k"] += 1
            return _o(a, b, m)

        girg_mod._torus_cell_gap = counting_gap
        try:
            sample_girg_adjacency(points, weights, ALPHA, np.random.default_rng(77))
        finally:
            girg_mod._torus_cell_gap = original
        evaluated[n] = calls["k"]

    assert evaluated[500] > 0, "the counter never fired — the test is not measuring anything"
    quadratic_ratio = (2000 * 1999) / (500 * 499)  # ~16
    ratio = evaluated[2000] / evaluated[500]
    assert ratio < 6.0, (
        "cell-pair work grew %.1fx over a 4x increase in n (quadratic would be %.1fx); "
        "the index is not buying what it should" % (ratio, quadratic_ratio))
