...
2. **Respect Mutability Frontmatter on Concept Files**:
   ...
   - `mutability: live` (status, next-actions, open-questions, lessons, all index files): Overwrite freely to reflect the current state — but truthfully. Don't mark something done that isn't; record blockers; a bundle full of optimistic fiction is worse than none.
   - `mutability: append-only` (`okf/log.md`, `changes/`, `decisions/`): Add new entries only. **CRITICAL**: You are strictly forbidden from using text editing tools to directly edit append-only files. You MUST use the provided Python script to append content: `python3 .agents/skills/edit-okf/scripts/append_okf.py <filepath> "<content>"` or pipe content into it (it also manages the read-only `chmod 444` lock on these files). Never edit prior entries.
...
3. **Keep index.md Synchronized**:
   ...
   - Update the respective `index.md` whenever a file in its directory is added, renamed, or removed.
