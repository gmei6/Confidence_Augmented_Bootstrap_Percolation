...
Rules:
1. Respect all FROZEN sections.
2. Follow repository structure in §5.3.
3. Python implementation is the unassailable source of truth for theoretical behavior.
4. C++ must match Python behavior according to the validation checks in §5.4.
5. Every experiment must be strictly reproducible.
6. Never introduce dense adjacency matrices.
7. Follow the Janson scaling regime in §4.
